package analytics //nolint:testpackage // test file lives alongside production code to access package internals
...
// errTest is a reusable sentinel for stub errors in repository tests.
...
func TestPostgresRepositoryRecordSnapshot(t *testing.T) {
	...
	mock.ExpectExec("INSERT INTO nav_snapshots").
		WithArgs(testPortfolioID, snapshotAt, 1000.0).
		WillReturnResult(sqlmock.NewResult(1, 1))

	err = repo.RecordSnapshot(context.Background(), testPortfolioID, snapshotAt, 1000.0)
	if err != nil {
...
func TestPostgresRepositoryRecordCashFlowWithSnapshot(t *testing.T) {
	...
	mock.ExpectQuery("INSERT INTO cash_flows").
		WithArgs(testPortfolioID, string(CashFlowDeposit), 500.0, "EUR", occurredAt).
		WillReturnRows(sqlmock.NewRows([]string{"id"}).AddRow(int64(7)))
	mock.ExpectExec("INSERT INTO nav_snapshots").
		WithArgs(testPortfolioID, occurredAt, 900.0, int64(7)).
		WillReturnResult(sqlmock.NewResult(1, 1))
	mock.ExpectCommit()

	err = repo.RecordCashFlowWithSnapshot(
		context.Background(), testPortfolioID, CashFlowDeposit, 500.0, "EUR", occurredAt, 900.0,
	)
	if err != nil {
...
func TestPostgresRepositoryRecordCashFlowWithSnapshotBeginError(t *testing.T) {
	...
	err = repo.RecordCashFlowWithSnapshot(
		context.Background(), testPortfolioID, CashFlowDeposit, 100.0, "EUR", occurredAt, 500.0,
	)
	if err == nil {
...
func TestPostgresRepositoryRecordCashFlowWithSnapshotInsertFlowError(t *testing.T) {
	...
	mock.ExpectBegin()
	mock.ExpectQuery("INSERT INTO cash_flows").WillReturnError(errTest)
	mock.ExpectRollback()
...
func TestPostgresRepositoryRecordCashFlowWithSnapshotInsertSnapshotError(t *testing.T) {
	...
	mock.ExpectExec("INSERT INTO nav_snapshots").WillReturnError(errTest)
	mock.ExpectRollback()
...
func TestPostgresRepositoryListSnapshots(t *testing.T) {
	...
	toTime := time.Date(2026, 1, 3, 0, 0, 0, 0, time.UTC)
	...
	rows := sqlmock.NewRows([]string{"id", "portfolio_id", "snapshot_at", "nav", "related_cash_flow_id"}).
		AddRow(int64(1), testPortfolioID, at1, 1000.0, nil).
		AddRow(int64(2), testPortfolioID, at2, 1050.0, int64(5))

	mock.ExpectQuery(regexp.QuoteMeta("FROM nav_snapshots")).
		WithArgs(testPortfolioID, from, toTime).
		WillReturnRows(rows)

	snaps, err := repo.ListSnapshots(context.Background(), testPortfolioID, TimeRange{From: from, To: toTime})
	if err != nil {
...
func TestPostgresRepositoryListSnapshotsError(t *testing.T) {
	...
	mock.ExpectQuery("FROM nav_snapshots").WillReturnError(errTest)

	_, err = repo.ListSnapshots(context.Background(), testPortfolioID, TimeRange{From: from, To: toTime})
	if err == nil {
		t.Fatal("expected error")
	}
}
...
func TestPostgresRepositorySignedCashFlowBetween(t *testing.T) {
	...
	total, err := repo.SignedCashFlowBetween(context.Background(), testPortfolioID, from, toTime)
	if err != nil {
...
func TestPostgresRepositorySignedCashFlowBetweenError(t *testing.T) {
	...
	mock.ExpectQuery("FROM cash_flows").WillReturnError(errTest)

	_, err = repo.SignedCashFlowBetween(context.Background(), testPortfolioID, from, toTime)
	if err == nil {
		t.Fatal("expected error")
	}
}
...
func TestPostgresRepositoryClearSnapshots(t *testing.T) {
	...
	mock.ExpectExec(regexp.QuoteMeta("DELETE FROM nav_snapshots WHERE portfolio_id = $1")).
		WithArgs(testPortfolioID).
		WillReturnResult(sqlmock.NewResult(0, 5))

	err = repo.ClearSnapshots(context.Background(), testPortfolioID)
	if err != nil {
