...
func TestEvaluateEntriesNoTrackedSymbols(t *testing.T) {
	t.Parallel()
	msgs := evaluateEntries(nil, nil, perSymbolConviction{}, Config{}, testNow)
	if len(msgs) != 0 {
...
func TestEvaluateEntriesSkipsTAADisabledSymbols(t *testing.T) {
	...
	holdings := []portfolio.Holding{
		{Symbol: "A", Quantity: 0, MarketPrice: 100, AllocationType: portfolio.AllocationSatellite, TAAEnabled: false},
	}
	msgs := evaluateEntries(holdings, []string{"A"}, perSymbolConviction{"A": 0.9}, Config{Buffer: 0.01}, testNow)
	if len(msgs) != 0 {
...
func TestEvaluateEntriesEmitsForUnownedHighConviction(t *testing.T) {
	...
	msgs := evaluateEntries(holdings, []string{"A", "B"}, perSymbolConviction{"A": 0.3, "B": 0.8}, Config{Buffer: 0.01}, testNow)
	if len(msgs) != 1 {
...
func TestEvaluateEntriesFrictionBlocksLowConviction(t *testing.T) {
	t.Parallel()
	msgs := evaluateEntries(nil, []string{"B"}, perSymbolConviction{"B": 0.005}, Config{BrokerFeePercent: 0.001, TaxRate: 0.26, Buffer: 0.01}, testNow)
	if len(msgs) != 0 {
...
func TestEvaluateEntriesDeltaEURScalesWithSatelliteNAV(t *testing.T) {
	...
	msgs := evaluateEntries(holdings, []string{"A", "B"}, perSymbolConviction{"A": 0.0, "B": 1.0}, Config{Buffer: 0.01}, testNow)
	if len(msgs) != 1 {
...
func TestEvaluateEntriesNegativeConvictionEmitsEntry(t *testing.T) {
	t.Parallel()
	msgs := evaluateEntries(nil, []string{"B"}, perSymbolConviction{"B": -0.8}, Config{Buffer: 0.01}, testNow)
	if len(msgs) != 1 {
...
func TestEvaluateEntriesEngineIntegration(t *testing.T) {
	...
	store := &fakeStore{holdings: []portfolio.Holding{
		{ID: 1, Symbol: "A", Quantity: 10, MarketPrice: 100, AllocationType: portfolio.AllocationSatellite, TAAEnabled: true},
	}}
	dispatch := &fakeDispatcher{}
	conv := perSymbolConviction{"A": 0.3, "B": 0.9}

	engine := NewEngine(store, &NullPMCReader{}, conv, &fakeSymbolProvider{syms: []string{"A", "B"}},
		dispatch, Config{TaxRate: 0.26, BrokerFeePercent: 0.001, Buffer: 0.01})
